...
task main()
{
	while (true)
	{
		...
		// Direction
		int direction = 1;
		if (vexRT[Btn6D] == 1 && direction == 1)
		{
			 direction = -1;
		}
		else if (vexRT[Btn6D] == 1 && direction == -1)
		{
			 direction = 1;
		}
		...
		if(vexRT[Btn6U]==1){
			...
			motor[motorLeft] = direction * gear * vexRT[Ch3];
			motor[motorRight] = -direction * gear * vexRT[Ch2];
		}
		...
		if (vexRT[Btn7L] == 1 && autoActive == false)
		{
			autoActive = true;
		}
