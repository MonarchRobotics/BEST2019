...
#pragma config(Motor,  port2,           leftMotor,     tmotorVex393_HBridge, openLoop, encoderPort, None)
#pragma config(Motor,  port3,           rightMotor,    tmotorVex393_HBridge, openLoop, encoderPort, None)
...
task main()
{
	while (true)
	{
		...
		if (vexRT[Btn7D] == 1)
		{
			while (getIrReading(irMiddle) < 0.5)
			{
				motor[leftMotor] = 100;
				motor[rightMotor] = -100;
				wait10Msec(1);

				if (getIrReading(irLeft) == 0)
				{
					motor[leftMotor] = -50;
					motor[rightMotor] = -50;
					wait1Msec(1);
